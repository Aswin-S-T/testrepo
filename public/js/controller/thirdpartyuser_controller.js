...
angular.module('F1FeederApp.controllers').
	...
	//		$scope.newapiKey = "";
		...
		$scope.saveThirdPartyUser = function(newName,newLimit) {
			
			

			if(newName == null||newLimit == null || newName == "" ||newLimit == "")
			{
			...
			{
				newNameregexExpression = /^[A-Za-z]{3,10}[._]{0,1}[0-9]{0,4}$/;
				let newNameregexResult = newNameregexExpression.test($scope.newName)
				if(newNameregexResult  == false)
				{
					alert("Invalid name")
					return;
				}
				...
				ergastAPIservice.saveThirdPartyUser(newThirdPartyUserObj, function (err) {
					if (!err) {
						...
						ergastAPIservice.getThirdPartyUserAt(null, 0, function (ThirdPartyUserInfo) {
