...
angular.module('F1FeederApp.controllers').
    ...
    controller('heatmapController', function heatmapController($scope, $sce, $routeParams, $location, $interval, $timeout, $localStorage, ergastAPIservice) {
        ...
        $scope.heatmap = {
            center: {
                latitude: 8.5241,//10.016033,
                longitude: 76.9366 //76.338835
            },
            zoom: 10,
            deviceMarkers: [],
            control: {},
        ...
        var maxRightParamsDisplayCount = 130;
                    ...
                    for (var j = 0; j < paramDefinitions.length; j++) {
                        ...
                        if (result != null && result.length > 0) {
                            ...
                            if (paramDefinition.paramName == "receivedTime")
                                continue;
                            if (paramDefinition.paramName == "time")
                                continue;
        ...
        var selectDevice = function (deviceId) {
            ...
            for (let j = 0; j < 15; j++) {
                if ($scope.deviceParams[j].paramName == 'AQI') {
                    

                    if ($scope.deviceParams[j].average.value <= 50) {
                        if ($scope.heatmap != null && $scope.heatmap.deviceMarkers != null) {
                            for (let i = 0; i < $scope.heatmap.deviceMarkers.length; i++) {
                                var thisMarker = $scope.heatmap.deviceMarkers[i];
                                
                                thisMarker.icon = { url: (thisMarker.id == selDeviceInfo.deviceId ? 'images/map-marker-selected-small-good.png' : 'images/map-marker-normal-small.png'), scaledSize: { width: 40, height: 50 } };
                            }
                        }
                    }

                    else if ($scope.deviceParams[j].average.value >= 51 && $scope.deviceParams[j].average.value <= 100) {
                        if ($scope.heatmap != null && $scope.heatmap.deviceMarkers != null) {
                            for (let i = 0; i < $scope.heatmap.deviceMarkers.length; i++) {
                                var thisMarker = $scope.heatmap.deviceMarkers[i];
                                
                                thisMarker.icon = { url: (thisMarker.id == selDeviceInfo.deviceId ? 'images/map-marker-selected-small-sat.png' : 'images/map-marker-normal-small.png'), scaledSize: { width: 40, height: 50 } };
                            }
                        }
                    }

                    else if ($scope.deviceParams[j].average.value >= 101 && $scope.deviceParams[j].average.value <= 200) {
                        if ($scope.heatmap != null && $scope.heatmap.deviceMarkers != null) {
                            for (let i = 0; i < $scope.heatmap.deviceMarkers.length; i++) {
                                var thisMarker = $scope.heatmap.deviceMarkers[i];
                                
                                thisMarker.icon = { url: (thisMarker.id == selDeviceInfo.deviceId ? 'images/map-marker-selected-small-mod.png' : 'images/map-marker-normal-small.png'), scaledSize: { width: 40, height: 50 } };
                            }
                        }
                    }

                    else if ($scope.deviceParams[j].average.value >= 201 && $scope.deviceParams[j].average.value <= 300) {

                        if ($scope.heatmap != null && $scope.heatmap.deviceMarkers != null) {
                            for (let i = 0; i < $scope.heatmap.deviceMarkers.length; i++) {
                                var thisMarker = $scope.heatmap.deviceMarkers[i];
                               
                                thisMarker.icon = { url: (thisMarker.id == selDeviceInfo.deviceId ? 'images/map-marker-selected-small-poor.png' : 'images/map-marker-normal-small.png'), scaledSize: { width: 40, height: 50 } };
                            }
                        }
                    }

                    else if ($scope.deviceParams[j].average.value >= 301 && $scope.deviceParams[j].average.value <= 400) {
                        if ($scope.heatmap != null && $scope.heatmap.deviceMarkers != null) {
                            for (var i = 0; i < $scope.heatmap.deviceMarkers.length; i++) {
                                var thisMarker = $scope.heatmap.deviceMarkers[i];
                               
                                thisMarker.icon = { url: (thisMarker.id == selDeviceInfo.deviceId ? 'images/map-marker-selected-small-vpoor.png' : 'images/map-marker-normal-small.png'), scaledSize: { width: 40, height: 50 } };
                            }
                        }
                    }

                    else if ($scope.deviceParams[j].average.value >= 401 && $scope.deviceParams[j].average.value <= 500) {
                        if ($scope.heatmap != null && $scope.heatmap.deviceMarkers != null) {
                            for (var i = 0; i < $scope.heatmap.deviceMarkers.length; i++) {
                                var thisMarker = $scope.heatmap.deviceMarkers[i];
                               
                                thisMarker.icon = { url: (thisMarker.id == selDeviceInfo.deviceId ? 'images/map-marker-selected-small-severe.png' : 'images/map-marker-normal-small.png'), scaledSize: { width: 40, height: 50 } };
                            }
                        }
                    }
