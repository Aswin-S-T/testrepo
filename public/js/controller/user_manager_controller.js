angular.module('F1FeederApp.controllers').
	...
	controller('userManagementController', function ($scope, $routeParams, $location, $rootScope, $sessionStorage, ergastAPIservice) {
		...
		$scope.saveUser = function (newName, newEmail, newContact, newRole, newUserName, newPassword, newPassword2) {
			...
			newNameregexExpression = /^[A-Za-z ]{3,20}$/;
			let newNameregexResult = newNameregexExpression.test(newName)
			
			if(newNameregexResult == false)
			{
				alert("Invalid name!!!Name should contain capital and small lettrs and l")
				return;
			}
			...
			if(/^\d{10}$/.test(newContact) == false){
				alert("Provide a valid 10 digit Contact number")
				return
			}
